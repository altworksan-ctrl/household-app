...
import { monthRange, money, tapeHex, computeSettlements, dateKeyOf, pad } from "../lib/helpers";
...
export default function MoneyTab({
  ...
  const [payer, setPayer] = useState(currentMember?.id || "");
  ...
  useEffect(() => {
    if (currentMember?.id) setPayer(currentMember.id);
  }, [currentMember]);
  ...
  const balances = activeMembers.map((m) => {
    ...
    return { id: m.id, name: m.name, color: tapeHex(m), net: paid - share };
  });
  ...
  return (
    <div className="px-4 pt-4 pb-24">
      ...
      <SectionCard className="mb-3">
        ...
        {myBalance && (
          <div className="mt-3 rounded-xl px-3 py-2.5 flex items-center justify-between" style={{ background: "var(--paper)" }}>
            <span className="text-sm font-medium">Your balance</span>
            <span className="font-mono font-bold" style={{ color: myBalance.net < -0.01 ? "var(--rust)" : "var(--moss)" }}>
              {myBalance.net >= 0 ? "+" : ""}
              {money(myBalance.net)}
            </span>
          </div>
      ...
      <div className="space-y-1.5 mb-4">
        ...
        {expenses.map((e) => {
          ...
          const canDelete = isAdmin || e.member_id === currentMember?.id;
          return (
      ...
      {settlements.length > 0 && (
        <>
          <div className="text-[10px] uppercase font-semibold text-[var(--ink-soft)] mb-1.5 px-1">Settle up</div>
          <SectionCard className="space-y-2">
            {settlements.map((s, i) => (
              <div key={i} className="flex items-center justify-between text-sm">
                <span>
                  <span className="font-semibold">{s.from}</span>
                  <span className="text-[var(--ink-soft)]"> pays </span>
                  <span className="font-semibold">{s.to}</span>
                </span>
                <span className="font-mono font-semibold">{money(s.amount)}</span>
              </div>
            ))}
          </SectionCard>
        </>
